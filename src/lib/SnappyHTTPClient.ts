...
export class SnappyHTTPClient extends BaseService {
	...
	async createOrganization(dto: CreateOrganizationDto) {
		...
		const authenticationResourceOrganization = await this.post<
	...
	): Promise<AuthenticationResourceOrganization> {
		this.refreshApiInstance(this.basePath, false);
		const authenticationResourceOrganization = await this.post<
			AuthenticateOrganizationDto,
			AuthenticationResourceOrganization
		>(dto, "/auth/organization");
		this.setBearerToken(authenticationResourceOrganization.token);
		this.setProjectId(authenticationResourceOrganization.data.projectId!);
		this.refreshApiInstance(this.basePath, true, this.bearerToken!);
		this.saveBearerToken();
	...
	): Promise<AuthenticationResourceUser> {
		this.refreshApiInstance(this.basePath, false);
		const authenticationResourceUser = await this.post<
			AuthenticateUserDto,
	...
	async getChatbotsForProject(projectId: string): Promise<Chatbot[]> {
		...
		return this.get<Chatbot[]>(
			`/chatbot/project-chatbot/:projectId?projectId=${projectId}`
		);
	}
	...
	async savePreKeyBundle(
		userId: string,
		dto: PreKeyBundle
	): Promise<PreKeyBundle> {
		this.refreshApiInstance(this.basePath, true, this.bearerToken!);
		return this.post<PreKeyBundle, Record<string, string>>(
			dto,
			`/signal/pre-key-bundle/${userId}`
		);
	}
	...
	async getUserContacts(dto: GetUserContactsDto): Promise<User[]> {
		this.refreshApiInstance(this.basePath, true, this.bearerToken!);
		return this.post<GetUserContactsDto, User[]>(
			dto,
			"/users/get-contacts"
		);
	}

	async filterUserByDisplayName(
		dto: FindUserByDisplayNameDto
	): Promise<User[]> {
		this.refreshApiInstance(this.basePath, true, this.bearerToken!);
		return this.post<FindUserByDisplayNameDto, User[]>(
			dto,
			"/users/filter/display-name"
		);
	}
	...
	async sendMessage(dto: SendMessageDto): Promise<Message> {
		...
		return await this.post<SendMessageDto, Message>(
			dto,
			"/chat/send",
			headers
		);
	...
	async getUserChats(
		...
		this.refreshApiInstance(this.basePath, true, this.bearerToken!);
		return await this.get<ChatResource[]>(
			`/chat/${user}/chats?projectId=${projectId}`
		);
	}
}