...
public class UpdateMessageAck implements UseCase<UpdateMessageAckDto, Message> {
    ...
    public Message execute(UpdateMessageAckDto dto) {
        ...
        if (message.isEmpty())
            throw new EntityNotFoundException("Target message not found; We are unable to change his ack");
        if (message.get().getAck().ordinal() + 1 == dto.getNewAck().ordinal()) {
            message.get().setAck(dto.getNewAck());
            messageRepository.save(message.get());
        } else {
            throw new IllegalStateTransitionException("Vous ne pouvez pas changer un ack de " + message.get().getAck() + " à " + dto.getNewAck());
        }
        return message.get();
    }
}
